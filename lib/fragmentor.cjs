...
function generateFragmentAmounts(totalAmount, numFragments) {
  ...
  // Generate random fragments that sum to total
  for (let i = 0; i < numFragments - 1; i++) {
    // Random variation: ±15% from average
    ...
    // Round to 2 decimals, ensure positive
    const amount = Math.max(0.1, Math.round(fragmentAmount * 100) / 100);
    fragments.push(amount);
    remaining -= amount;
  }
  
  // Last fragment gets remaining (ensures exact total)
...
function generateFragmentSecrets(numFragments) {
  ...
  for (let i = 0; i < numFragments; i++) {
    ...
    secrets.push({
      ...
      secret: secret,
      secretId: secretId,
      secretBigInt: BigInt(secret).toString()
    });
  }
