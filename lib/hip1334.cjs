...
/**
 * Derive the X25519 public key from a raw 32-byte private key hex string.
 * Returns the raw 32-byte public key as a hex string.
 */
function x25519PublicKeyFromPrivate(privateKeyHex) {
  // Wrap raw private key in PKCS8 so Node.js accepts it
  const privKey = crypto.createPrivateKey({
    key: _buildPKCS8(Buffer.from(privateKeyHex, 'hex')),
    format: 'der',
    type: 'pkcs8',
  });
  // Export the corresponding public key as DER SPKI and slice the last 32 bytes
  const pubKey = crypto.createPublicKey(privKey);
  const pubDer = pubKey.export({ type: 'spki', format: 'der' });
  return pubDer.slice(-32).toString('hex');
}
